...
function BookingPage() {
  ...
  const downloadICS = () => {
    ...
    const summary = `Screening Call — ${role.job_title} at ${role.company_name}`;
    const description = `15-minute screening call for ${role.job_title} at ${role.company_name}.${booking.meet_link ? `\n\nJoin Meeting: ${booking.meet_link}` : ''}`;
    const lines = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      ...
      'METHOD:PUBLISH',
      'BEGIN:VEVENT',
      `UID:${booking.id}-${Date.now()}@screening-platform`,
      `DTSTART:${startStr}`,
      `DTEND:${endStr}`,
      `SUMMARY:${escaped(summary)}`,
      `DESCRIPTION:${escaped(description)}`,
      ...(booking.meet_link ? [`LOCATION:${escaped(booking.meet_link)}`] : []),
      'END:VEVENT',
      'END:VCALENDAR'
    ];
